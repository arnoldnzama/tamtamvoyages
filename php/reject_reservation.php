...
try {
    ...
    $check_sql = "SELECT id, token_expiry FROM reservations 
                  WHERE id = ? AND reject_token = ? AND statut = 'En attente'";
    ...
    if ($check_result->num_rows === 0) {
        throw new Exception('Le lien de rejet est invalide ou a déjà été utilisé');
    }
    ...
    if (strtotime($reservation['token_expiry']) < time()) {
        throw new Exception('Le lien de rejet a expiré');
    }
    ...
    if ($reservation) {
        ...
        $message = "Bonjour " . htmlspecialchars($reservation['prenom']) . " " . htmlspecialchars($reservation['nom']) . ",\n\n" .
                  "Nous regrettons de vous informer que votre réservation a été rejetée :\n\n" .
        ...
        if (!mail($reservation['email'], "Votre réservation a été rejetée - TamtamVoyages", $message, $headers)) {
            error_log("Erreur lors de l'envoi de l'email de rejet pour la réservation #" . $id);
        }
    }

    displayMessage(
        'Réservation rejetée',
        'La réservation a été rejetée avec succès. Un email a été envoyé au client.',
        'success'
    );

} catch (Exception $e) {
    error_log("Erreur dans reject_reservation.php : " . $e->getMessage());
